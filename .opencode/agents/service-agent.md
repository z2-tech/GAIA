...
- **Auto-complete**: `ProjectService.auto_complete_project(project)` (projects/services.py) — guards: status must be `in_progress`, farm_count > 0; if completion == 100 → `status=completed`, `save(update_fields=["status"])`.
- **Hooks (8)**: `routhc/services.py::RouthcService.calcular` (pós-create), `regenerative/services.py::create_assessment`/`update_assessment` (pós-save), `lca/views.py` 5 mutations (culture/soil/inputs/fuel/calculate via `_auto_complete_project`). NO signals — explicit check pós-mutation (HackSoftware).

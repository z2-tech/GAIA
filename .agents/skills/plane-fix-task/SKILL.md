...
name: plane-fix-task
description: >-
  Fix a bug described in a Plane work item. Use when the user hands over a Plane
  card — a URL or a human identifier like "GAIA-1" — reporting a defect to fix.
  Fetches the card from Plane, then hands the extracted spec to the
  `gaia-feature-dev` skill for the full analyze → plan → approval → implement
  workflow, framed as a bug fix. Triggers: "corrigir essa task do plane",
  "fix card GAIA-1", "resolver esse bug do plane", "fix plane task",
  "/plane-fix-task".
---
...
workflow against it as a **bug fix** (restore intended behavior, minimal blast
radius). For building new behavior, use `plane-develop-task`.
...
2. **Extract the spec.** From the retrieved item read: `name`, `description_html`
   ...
   For a bug the **Problema** is the defect (expected vs actual); the Checklist is
   the acceptance criteria for "fixed".
...
3. **Confirm target.** Echo back: identifier + title + state + assignee, and a
   one-line restatement of the defect you're about to fix. Sanity check, not an
   approval gate.
4. **Hand off to `gaia-feature-dev`.** Invoke the `gaia-feature-dev` skill,
   ...
   - framing: **bug fix** — prefer the smallest change that restores intended
     behavior; call out a regression test in the plan.
...
- Read-only resolution here — this skill fetches and frames; it does **not** edit
  product source or move the card's state. All fix work goes through
